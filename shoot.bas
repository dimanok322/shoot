...
120 IF MID$(A$,1,1)<>"Y" THEN GOTO 440
130 PRINT
...
620 NEXT Y
630 NEXT X
640 IF A=2 THEN GOTO 690
650 FOR X=1 TO 8
660 READ C,D
670 IF C=2 THEN GOTO 690
680 NEXT X
690 FOR X=1 TO 8
700 FOR Y=1 TO 2
710 READ P(X,Y)
...
770 IF E<1 OR E>10 OR F<1 OR F>10 THEN GOTO 760
780 Z=1
790 I(E,F)=1
800 H(A,B)=1
810 PRINT
...
840 IF M<1 OR M>10 OR N<1 OR N>10 THEN GOTO 830
850 INPUT "WHERE TO MOVE TO";S,T
860 IF S<1 OR S>10 OR T<1 OR T>10 THEN GOTO 850
870 IF I(S,T) = 1 THEN GOTO 850
880 FOR X=1 TO 8
890 IF P(X,1)+E=S AND P(X,2)+F=T THEN GOTO 920
900 NEXT X
910 GOTO 850
920 PRINT
930 L=1
940 FOR X=1 TO 8
950 IF P(X,1)+E>10 OR P(X,1)+E<1 OR P(X,2)+F>10 OR P(X,2)+F<1 THEN GOTO 1000
960 IF  I(P(X,1)+E,P(X,2)+F)=1 THEN GOTO 1000
970 T(L,1)=P(X,1)+E
980 T(L,2)=P(X,2)+F
990 L=L+1
1000 NEXT X
1010 L=L-1
1020 IF L<>1 THEN GOTO 1060
1030 C=T(L,1)
1040 D=T(L,2)
1050 GOTO 1090
...
1120 IF H(P(X,1)+A,P(X,2)+B)=1 THEN GOTO 1160
1130 T(L,1)=P(X,1)+A
1140 T(L,2)=P(X,2)+B
1150 L=L+1
1160 NEXT X
1170 L=L+1
1180 IF L<>0 THEN GOTO 1220
1190 PRINT C$;"THE ENEMY HAS CORNERED HIMSELF IN!!"
1200 Z=0
1204 C=E
1207 D=F
1210 GOTO 1290
1220 IF L<>1 THEN GOTO 1260
1230 J=T(1,1)
1240 K=T(1,2)
1250 GOTO 1290
...
1310 I(C,D)=1
1320 H(M,N)=1
1330 IF M<>J OR D<>T THEN GOTO 1390
1340 PRINT C$J"HEY! YOU GOT HIM!!"
1350 Z=0
1360 IF C<>S OR D<>T THEN GOTO 1390
...
1420 B=K
1430 FOR X=1 TO 8
1440 IF P(X,1)+E>10ORP(X,1)+E<1 OR P(X,2)+F>10ORP(X,2)+F<1 THEN GOTO 1460
1450 IF I(P(X,1)+E,P(X,2)+F)=0 THEN GOTO 1490
1460 NEXT X
...
1510 PRINT G$;S$;G$
1520 FOR X=1 TO 10
1530 IF X=10 THEN GOTO 1560
1540 PRINT X;
1550 GOTO 1570
1560 PRINT "10 ";
1570 FOR Y=1 TO 10
1580 IF I(X,Y)=1 THEN GOTO 1610
1590 PRINT ":";
1600 GOTO 1620
1610 PRINT "*";
1620 NEXT Y
1630 PRINT " ";S$;
1640 IF X=10 THEN GOTO 1670
1650 PRINT X;
1660 GOTO 1680
1670 PRINT "10 ";
1680 FOR Y=1 TO 10
1690 IF H(X,Y)=1 THEN GOTO 1720
1700 PRINT ":";
1710 GOTO 1730
1720 PRINT "*";
1730 NEXT Y
